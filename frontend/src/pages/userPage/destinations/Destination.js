...
const Destination = () => {
  ...
  useEffect(() => {
    setFilteredDestinations(filterDestinations(destinations, selectedCategory, searchTerm));
  ...
  const filterDestinations = (destinations, category, searchTerm) => {
    ...
    if (category !== 'All') {
      filtered = filtered.filter((destination) => destination.category === category);
    }
  ...
  return (
    ...
    <header className="TopNavigation">
      <div className="Logo">Explore</div>
        ...
        <div className="FilterContainer">
          <label htmlFor="categoryFilter"><h3>Filter by Category:</h3></label>
          <select
           id="categoryFilter"
    ...
    <div className="MainContent">
      {filteredDestinations.length === 0 && searchTerm !== '' && <p>No results found! search another Destinations</p>}
      {filteredDestinations.map((destination) => (
       <div key={destination._id} className="DestinationCard">
          <h3>{destination.name}</h3>
          <p className="label">Category: <span>{destination.category}</span></p>
          <p className="label">Location: <span>{destination.location}</span></p>
